#!/usr/bin/env python3
"""Local Ollama + MCP health/habit agent.

Bridges Ollama chat completions to the local health-tracker MCP server.

Examples:
  python health_agent.py "Create habits for walking, water, and stretching, then log today's walk."
  python health_agent.py --model qwen2.5:7b
"""
...
SERVER_PATH = PROJECT_DIR / "health_server.py"

SYSTEM_PROMPT = """You are a local health and habit tracking assistant.
Use MCP tools to create habits and log workouts, meals, habits, and measurements.
Be concise, practical, and non-judgmental. Do not provide medical diagnoses or medication advice.
If the user asks for medical advice, suggest contacting a qualified healthcare professional.
Use YYYY-MM-DD dates. Confirm what was saved."""
...
async def run_agent(prompt: str, model: str, data_dir: Path | None) -> str:
    env = os.environ.copy()
    if data_dir:
        env["MCP_HEALTH_DIR"] = str(data_dir)
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            print("[agent] connected to health MCP server; tools:", ", ".join(t.name for t in tools_response.tools), file=sys.stderr)
...
async def repl(model: str, data_dir: Path | None) -> None:
    print("Local Ollama MCP health/habit tracker. Type 'exit' or Ctrl-C to quit.", file=sys.stderr)
    while True:
        try:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a local Ollama-powered MCP health/habit agent.")
    parser.add_argument("prompt", nargs="?", help="Prompt to run once. Omit for interactive mode.")
    parser.add_argument("--model", default=DEFAULT_MODEL, help=f"Ollama model (default: {DEFAULT_MODEL})")
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=Path(os.environ.get("MCP_HEALTH_DIR", Path.home() / "MCPHealth")),
        help="Directory for health data",
    )
