#!/usr/bin/env python3
"""Local Ollama + MCP flashcards / spaced-repetition agent.

Bridges Ollama chat completions to the local flashcards MCP server.

Examples:
  python flashcards_agent.py "Make a deck of 8 cards covering MCP basics."
  python flashcards_agent.py --model qwen2.5:7b
"""
...
SERVER_PATH = PROJECT_DIR / "flashcards_server.py"

SYSTEM_PROMPT = """You are a local flashcards and spaced-repetition study assistant.
Use the MCP tools to build decks, add cards, and run review sessions.

When making cards, write a single clear question on the front and one concise answer on the back.
Base every card only on facts the user gave you or on well-established knowledge; never invent
dates, numbers, or citations. Prefer many small cards over a few dense ones.

When running a review: call get_due_cards first, show the user each front one at a time, and ask
them to answer before revealing the back. Then ask them to rate their recall from 0 to 5
(0 = blackout, 3 = correct but hard, 5 = perfect) and pass that exact number to record_review.
Never grade a card on the user's behalf.

Report what was created and, after reviews, the next due dates. Use YYYY-MM-DD dates."""
...
async def run_agent(prompt: str, model: str, data_dir: Path | None) -> str:
    env = os.environ.copy()
    if data_dir:
        env["MCP_FLASHCARDS_DIR"] = str(data_dir)
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            print("[agent] connected to flashcards MCP server; tools:", ", ".join(t.name for t in tools_response.tools), file=sys.stderr)
...
async def repl(model: str, data_dir: Path | None) -> None:
    print("Local Ollama MCP flashcards tutor. Type 'exit' or Ctrl-C to quit.", file=sys.stderr)
    while True:
        try:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a local Ollama-powered MCP flashcards agent.")
    parser.add_argument("prompt", nargs="?", help="Prompt to run once. Omit for interactive mode.")
    parser.add_argument("--model", default=DEFAULT_MODEL, help=f"Ollama model (default: {DEFAULT_MODEL})")
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=Path(os.environ.get("MCP_FLASHCARDS_DIR", Path.home() / "MCPFlashcards")),
        help="Directory for flashcard decks and review history",
    )
