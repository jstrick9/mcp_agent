#!/usr/bin/env python3
"""Local Ollama + MCP planning agent.

Bridges Ollama chat completions to the local MCP planner server over stdio.

Examples:
  python planner_agent.py "Create a weekend project plan for cleaning the garage."
  python planner_agent.py --model qwen2.5:7b
"""
...
SERVER_PATH = PROJECT_DIR / "planner_server.py"

SYSTEM_PROMPT = """You are a local planning assistant.
Use the MCP planner tools to create/update projects and tasks. Keep task titles short.
Store details, checklists, and decisions in project notes. Confirm what you saved.
Do not claim a task exists unless a tool call returned it. Use YYYY-MM-DD dates."""
...
async def run_agent(prompt: str, model: str, data_dir: Path | None) -> str:
    env = os.environ.copy()
    if data_dir:
        env["MCP_PLANNER_DIR"] = str(data_dir)
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            print("[agent] connected to planner MCP server; tools:", ", ".join(t.name for t in tools_response.tools), file=sys.stderr)
...
async def repl(model: str, data_dir: Path | None) -> None:
    print("Local Ollama MCP planner. Type 'exit' or Ctrl-C to quit.", file=sys.stderr)
    while True:
        try:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a local Ollama-powered MCP planning agent.")
    parser.add_argument("prompt", nargs="?", help="Prompt to run once. Omit for interactive mode.")
    parser.add_argument("--model", default=DEFAULT_MODEL, help=f"Ollama model (default: {DEFAULT_MODEL})")
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=Path(os.environ.get("MCP_PLANNER_DIR", Path.home() / "MCPPlanner")),
        help="Directory for planner projects/tasks",
    )
